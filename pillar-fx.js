/* ============================================================
   ETHOS EMPIRE — PILLAR-FX.JS  v4
   All pillar section canvas animations.
   Every animation is a perfectly seamless 20-second loop.
   All canvases are transparent — matrix background shows through.

   SEAMLESS GUARANTEE: every time-varying value uses
   integer multiples of tf*TAU (or tf*360 for hue), so
   state at tf=0 is mathematically identical to tf=1.
   Zero visual jumps on loop restart.

   Sections:
     #discipline-anim  → Brain neuron network
     #confidence-anim  → 3D colour-cycling icosahedron
     #health-anim      → Low-poly 3D heart + octagon rings
     #legacy-anim      → 3D king crown + hexagonal rings
   ============================================================ */
(function () {
  'use strict';
  ...
  /* ─────────────────────────────────────────────────────────
     SHARED: Deterministic seeded hash  sr(n) → 0…1
     ───────────────────────────────────────────────────────── */
  function sr(n) {
  ...
  function startDiscipline(wrap) {
    ...
    const NUM_N=34,NUM_P=60; let cW,cH,scene;
  ...
  function startConfidence(wrap) {
    ...
    function frame(ts){const T=ts%LOOP,tf=T/LOOP,a=tf*TAU,gh=tf*360;ctx.clearRect(0,0,cW,cH);const pulse=.5+.5*Math.sin(tf*TAU);const ag=ctx.createRadialGradient(CX,CY,icoScale*.05,CX,CY,icoScale*(1.6+.2*pulse));ag.addColorStop(0,`hsla(${gh},80%,25%,${(.1+.03*pulse).toFixed(3)})`);ag.addColorStop(.55,`hsla(${gh},60%,15%,0.04)`);ag.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=ag;ctx.fillRect(0,0,cW,cH);drawEnergyRing(tf,gh);drawPoly(OVERTS,OFACES,-a*2,a,0,octScale,(gh+180)%360);drawPoly(IVERTS,IFACES,a,.44,0,icoScale,gh);drawOrbiters(tf,gh);drawDust(tf,gh);requestAnimationFrame(frame);}
    requestAnimationFrame(frame);
  }

  /* ============================================================
     ANIMATION 3 — HEALTH
     Low-poly 3D heart + octagon rings.
     SEAMLESS FIXES: ring Y-follow uses tf*TAU (integer, was 0.45×)
     ============================================================ */
  function startHealth(wrap) {
    ...
    /* Octagon rings — sPd is integer, Y-follow uses tf*TAU (seamless) */
    ...
    function orbitDisp(q,tf){const sc3=(p,s)=>({x:p.x*s,y:p.y*s,z:p.z*s}),ZERO={x:0,y:0,z:0};
      const posAt=(t)=>{const a=q.oPh+t*TAU*q.oSpd;let p={x:q.oRad*Math.cos(a),y:0,z:q.oRad*Math.sin(a)};p=rx(p,q.oInc);p=ry(p,q.oAsc);return p;};
      const flyOrigin=posAt(PH.ORBIT);
      if(tf<PH.SPIN)return ZERO;if(tf<PH.ORBIT_IN)return sc3(posAt(tf),sm(tf,PH.SPIN,PH.ORBIT_IN-PH.SPIN));if(tf<PH.ORBIT)return posAt(tf);
      if(tf<PH.FLY)return sc3(flyOrigin,1+sm(tf,PH.ORBIT,PH.FLY-PH.ORBIT)*3.5);
      return sc3(flyOrigin,4.5*(1-sm(tf,PH.FLY,PH.DONE-PH.FLY)));}
    /* SEAMLESS FIX: ring Y-follow = tf*TAU (1 full rotation, integer) */
    function drawOctaRings(tf,orbitAmt,flyAmt,angY){
      const gh=tf*360,baseA=0.22+orbitAmt*0.55+flyAmt*0.15;
      RINGS.forEach((ring,ri)=>{const ringR=ring.rFrac,rotAng=tf*TAU*ring.sPd,verts=[];
        for(let i=0;i<=N_OCT;i++){const a=rotAng+(i/N_OCT)*TAU;let p={x:ringR*Math.cos(a),y:0,z:ringR*Math.sin(a)};p=rx(p,ring.tX);if(ring.tZ)p=rz(p,ring.tZ);
          p=ry(p,tf*TAU);  /* FIXED: was angY*0.45, now integer-multiple */
          verts.push(p);}
        for(let i=0;i<N_OCT;i++){const v0=verts[i],v1=verts[i+1],avgZ=(v0.z+v1.z)*0.5;const dA=Math.max(0.12,Math.min(1,0.72-avgZ/(ringR*0.9)));const hue=(gh+ri*120+i*(360/N_OCT))%360,finalA=(baseA*dA).toFixed(3);const p0=proj(v0),p1=proj(v1);const GAP=0.12,sx=p0.x+(p1.x-p0.x)*GAP,sy=p0.y+(p1.y-p0.y)*GAP,ex=p0.x+(p1.x-p0.x)*(1-GAP),ey=p0.y+(p1.y-p0.y)*(1-GAP);ctx.beginPath();ctx.moveTo(sx,sy);ctx.lineTo(ex,ey);ctx.strokeStyle=`hsla(${hue.toFixed(0)},92%,65%,${finalA})`;ctx.lineWidth=1.8;ctx.shadowBlur=7;ctx.shadowColor=`hsl(${hue.toFixed(0)},100%,60%)`;ctx.stroke();ctx.shadowBlur=0;ctx.beginPath();ctx.arc(p0.x,p0.y,3.0,0,TAU);ctx.fillStyle=`hsla(${hue.toFixed(0)},100%,78%,${finalA})`;ctx.shadowBlur=10;ctx.shadowColor=`hsl(${hue.toFixed(0)},100%,70%)`;ctx.fill();ctx.shadowBlur=0;}});}
    ...
    function frame(ts){const tf=(ts%LOOP)/LOOP;ctx.clearRect(0,0,cW,cH);const angY=tf*TAU,angX=0.28+0.08*Math.sin(tf*TAU*2),rot=(v)=>{let p=rx(v,angX);return ry(p,angY);};const inOrbit=(tf>=PH.ORBIT_IN&&tf<PH.ORBIT)?sm(tf,PH.ORBIT_IN,0.05):0,flyAmt=(tf>=PH.ORBIT&&tf<PH.FLY)?sm(tf,PH.ORBIT,PH.FLY-PH.ORBIT):0;const gI=0.07+inOrbit*0.08+flyAmt*0.16,gR=SC*(1.4+inOrbit*1.0+flyAmt*3.2),gGrad=ctx.createRadialGradient(CX,CY,0,CX,CY,gR);gGrad.addColorStop(0,`rgba(255,30,70,${gI.toFixed(3)})`);gGrad.addColorStop(0.45,`rgba(160,0,30,${(gI*0.4).toFixed(3)})`);gGrad.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=gGrad;ctx.fillRect(0,0,cW,cH);drawOctaRings(tf,inOrbit,flyAmt,angY);const faceData=QUADS.map((q)=>{const pa=BV[q.a],pb=BV[q.b],pc=BV[q.c],pd=BV[q.d];const e1={x:pb.x-pa.x,y:pb.y-pa.y,z:pb.z-pa.z},e2={x:pd.x-pa.x,y:pd.y-pa.y,z:pd.z-pa.z};let nx=e1.y*e2.z-e1.z*e2.y,ny=e1.z*e2.x-e1.x*e2.z,nz=e1.x*e2.y-e1.y*e2.x;const nl=Math.hypot(nx,ny,nz)||1;nx/=nl;ny/=nl;nz/=nl;const fn={x:nx,y:ny,z:nz};const od=orbitDisp(q,tf),sh={x:od.x,y:od.y,z:od.z},ov=(v)=>({x:v.x+sh.x,y:v.y+sh.y,z:v.z+sh.z}),rva=rot(ov(pa)),rvb=rot(ov(pb)),rvc=rot(ov(pc)),rvd=rot(ov(pd)),rc=rot({x:((pa.x+pb.x+pc.x+pd.x)*.25)+sh.x,y:((pa.y+pb.y+pc.y+pd.y)*.25)+sh.y,z:((pa.z+pb.z+pc.z+pd.z)*.25)+sh.z}),rfn=rot(fn),diffuse=Math.max(0,0.55*rfn.z+0.28*rfn.y-0.18*rfn.x),isFront=rfn.z>=-0.02,pA=proj(rva),pB=proj(rvb),pC=proj(rvc),pD=proj(rvd),col=PAL[q.ci],orbitBoost=(inOrbit+flyAmt*0.5)*12,litAdj=isFront?diffuse*44+orbitBoost:-24,lit=Math.max(6,Math.min(86,col.l+litAdj)),alph=isFront?0.78+diffuse*0.20:(inOrbit>0.1||flyAmt>0.1?0.12:0.06);return{pA,pB,pC,pD,col,lit,alph,depth:rc.z,isFront,orbiting:inOrbit>0.05||flyAmt>0.05};});faceData.sort((a,b)=>a.depth-b.depth);faceData.forEach(f=>{const{h,s}=f.col;if(f.orbiting){const mx=(f.pA.x+f.pB.x+f.pC.x+f.pD.x)*.25,my=(f.pA.y+f.pB.y+f.pC.y+f.pD.y)*.25,gr=ctx.createRadialGradient(mx,my,0,mx,my,18);gr.addColorStop(0,`hsla(${h},${s}%,${f.lit.toFixed(0)}%,${((inOrbit+flyAmt)*0.10).toFixed(3)})`);gr.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=gr;ctx.fillRect(mx-20,my-20,40,40);}ctx.beginPath();ctx.moveTo(f.pA.x,f.pA.y);ctx.lineTo(f.pB.x,f.pB.y);ctx.lineTo(f.pD.x,f.pD.y);ctx.lineTo(f.pC.x,f.pC.y);ctx.closePath();ctx.fillStyle=`hsla(${h},${s}%,${f.lit.toFixed(0)}%,${f.alph.toFixed(2)})`;ctx.fill();if(f.isFront){ctx.strokeStyle=`hsla(${h},${s}%,${Math.max(5,f.lit*0.48).toFixed(0)}%,0.60)`;ctx.lineWidth=0.75;ctx.stroke();}});drawSparkles(tf);drawDust(tf);requestAnimationFrame(frame);}
    requestAnimationFrame(frame);
  }

  /* ============================================================
     ANIMATION 4 — LEGACY
     3D King Crown + hexagonal rings + luxury particles.
     SEAMLESS: all frequencies are integers × tf*TAU or tf*360.
     ============================================================ */
  function startLegacy(wrap) {
    ...
    /* 3 hexagonal rings — sPd integer, Y-follow uses tf*TAU (seamless) */
    ...
    /* SEAMLESS FIX: ring Y-follow uses tf*TAU (integer, was 0.40×) */
    function drawRings(tf,angY){
      const gh=tf*360;
      RINGS_L.forEach((ring,ri)=>{const ringR=ring.rFrac,rotAng=tf*TAU*ring.sPd,verts=[];
        for(let i=0;i<=N_HEX;i++){const a=rotAng+(i/N_HEX)*TAU;let p={x:ringR*Math.cos(a),y:0,z:ringR*Math.sin(a)};p=rx(p,ring.tX);if(ring.tZ)p=rz(p,ring.tZ);
          p=ry(p,tf*TAU);  /* FIXED: was angY*0.40, now tf*TAU (seamless) */
          verts.push(p);}
        for(let i=0;i<N_HEX;i++){const v0=verts[i],v1=verts[i+1],avgZ=(v0.z+v1.z)*0.5,dA=Math.max(0.15,Math.min(1,0.75-avgZ/(ringR*0.9)));
          /* Gold-biased hue: stays mostly 30–90° (gold range) */
          const hue=(30+((gh*0.7+ri*80+i*60)%120))%360;
          const p0=proj(v0),p1=proj(v1),GAP=0.10;
          const sx=p0.x+(p1.x-p0.x)*GAP,sy=p0.y+(p1.y-p0.y)*GAP,ex=p0.x+(p1.x-p0.x)*(1-GAP),ey=p0.y+(p1.y-p0.y)*(1-GAP);
          ctx.beginPath();ctx.moveTo(sx,sy);ctx.lineTo(ex,ey);ctx.strokeStyle=`hsla(${hue.toFixed(0)},90%,68%,${dA.toFixed(2)})`;ctx.lineWidth=2.0;ctx.shadowBlur=8;ctx.shadowColor=`hsl(${hue.toFixed(0)},100%,65%)`;ctx.stroke();ctx.shadowBlur=0;
          ctx.beginPath();ctx.arc(p0.x,p0.y,3.5,0,TAU);ctx.fillStyle=`hsla(${hue.toFixed(0)},100%,85%,${dA.toFixed(2)})`;ctx.shadowBlur=12;ctx.shadowColor=`hsl(${hue.toFixed(0)},100%,72%)`;ctx.fill();ctx.shadowBlur=0;}});}

    /* SEAMLESS FIX: spire spin uses tf*TAU*1 (was tf*TAU*0.5) */
    function drawSpireTips(tf,angY,angX,bob){
      const rot=(v)=>{let p=rx(v,angX);return ry(p,angY);};
      for(let s=0;s<5;s++){const u=s/5*TAU,tip={x:R_C*Math.cos(u),y:Y_TOP+bob,z:R_C*Math.sin(u)},rTip=rot(tip),pr=proj(rTip);
        const pulse=0.55+0.45*Math.sin(tf*TAU*3+s*TAU/5);  /* 3 cycles, seamless */
        const sz=SC*0.095*pulse;
        for(let k=0;k<8;k++){
          const a=k*Math.PI/4+tf*TAU;  /* FIXED: was tf*TAU*0.5, now tf*TAU*1 (seamless) */
          const len=k%2===0?sz:sz*0.55;
          ctx.beginPath();ctx.moveTo(pr.x,pr.y);ctx.lineTo(pr.x+Math.cos(a)*len,pr.y+Math.sin(a)*len);ctx.strokeStyle=`rgba(255,248,200,${(pulse*0.85).toFixed(2)})`;ctx.lineWidth=k%2===0?2.0:1.2;ctx.shadowBlur=10;ctx.shadowColor='#FFE87C';ctx.stroke();ctx.shadowBlur=0;}
        ctx.beginPath();ctx.arc(pr.x,pr.y,3.8*pulse,0,TAU);ctx.fillStyle=`rgba(255,255,240,${pulse.toFixed(2)})`;ctx.shadowBlur=16;ctx.shadowColor='#FFFACD';ctx.fill();ctx.shadowBlur=0;}}
    ...
    /* SEAMLESS FIX: gemlet hue drift = tf*360 (full spectrum = seamless, was tf*120) */
    function drawGemlets(tf){GEMLETS.forEach(s=>{const a=tf*TAU*s.spd+s.ph;let p={x:Math.cos(a)*s.rFrac,y:0,z:Math.sin(a)*s.rFrac};p=rx(p,s.tX);const pr=proj(p);
      const hue=(s.hOff+tf*360)%360;  /* FIXED: was tf*120, now tf*360 (full circle = seamless) */
      const pulse=0.65+0.35*Math.sin(tf*TAU*s.spd*2+s.ph);ctx.beginPath();ctx.arc(pr.x,pr.y,s.r*pulse,0,TAU);ctx.fillStyle=`hsla(${hue.toFixed(0)},95%,72%,0.80)`;ctx.shadowBlur=12;ctx.shadowColor=`hsl(${hue.toFixed(0)},100%,68%)`;ctx.fill();ctx.shadowBlur=0;});}
    ...
    function frame(ts){
      ...
      drawSpireTips(tf,angY,angX,bob);
      drawGemlets(tf);
      drawDust(tf);
      requestAnimationFrame(frame);}
    requestAnimationFrame(frame);
